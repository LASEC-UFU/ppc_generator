...
Componentes optativos adicionais, cursados além do elenco vinculado às três Ênfases Formativas — inclusive disciplinas de qualquer unidade acadêmica da UFU incorporadas mediante requerimento do interessado, após a anuência do Núcleo Docente Estruturante (NDE) e a aprovação do Colegiado do Curso — não substituem a exigência de integralização de, no mínimo, \input{gerado/par_enfases_formativas_minimas} das três ênfases formativas, sendo registrados como carga optativa excedente (\autoref{chp:modulos_optativos}, Seção~\ref{sec:regra_escolha}).
